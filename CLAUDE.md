...
- **测验**：基于阅读内容生成 5 道选择题，难度根据 JLPT 等级调整；历史记录含进度图表
  - **出题范围可见**：有选区则只用选区，否则用全文；按块边界累加至 4000 字（`lib/api.js` 的静默截断退化为兜底），面板顶部说明覆盖范围（`quizScopeSelected` / `quizScopeTruncated`），用到的块加紫色左边线 `.quiz-scope`
  - 窄屏（≤900px）从右侧抽屉改为**底部抽屉**，通过测量播放条高度叠在其上方
