...
| [08-business-case.md](08-business-case.md) | 商业评审：目标用户、竞品与定位、价值阶梯、单位经济与盈亏平衡、增长渠道、每阶段量化放行标准、法务税务与许可 | 贯穿 |

建议阅读顺序：本页 → 08（先看商业面站不站得住）→ 07（看全局排期）→ 01 → 02 → 03 → 04 → 05 → 06。
...
6. **免费档是否包含语法 / 测验。** 按现有数值免费用户一天做不了一次测验（`05` §1.2 第 4 点）。建议免费档走 flash 档、Pro 走中档。
7. **定价本地化。** 是否给东南亚 / 南亚市场折扣价（`08` §4.5）。
8. **仓库公开与许可。** 仓库当前公开且无 LICENSE 文件；`server/` 进同一仓库即后端开源。三个选项见 `08` §7.4。**影响 M0**（LICENSE 必须先补）。
9. **双品牌 Yomeru / Rubify 是否保留。** 用户在 Yomeru 里看到"登录 Rubify"的转化代价见 `08` §7.5。影响 M0 的 listing 文案与隐私政策域名。
10. **是否增加 Google 以外的登录方式。** Google 在大陆不可达；若决策门 1 含大陆则必须加邮件魔法链接（`04` §1.1）。
11. **各里程碑的放行数值。** `08` §6 给了建议值，需确认后作为 M0→M2、M2→M3 的硬门槛。

决策门 2 请连同"收款通道"一起回答：Stripe 直连需要自己做欧盟 OSS 等税务登记，MoR（Paddle / Lemon Squeezy / Polar）把这部分连同拒付一起接走，费率差异对海外用户不到 $0.3 / 单（`08` §7.1）。
...
| 上游模型费用 | 见 `05` 成本模型；M2 阶段建议设全局日预算上限 $10–20（即月敞口上限 $300–600） |
| 收款主体维护 | 走 MoR 以个人身份：$0；走 Stripe Atlas 美国 LLC：注册 $500 一次 + 每年约 $1,000–2,000 税务与代理 |
| 可公开的联系地址（M3 trader 声明） | 虚拟地址约 $10–30 / 月 |

单位经济、盈亏平衡转化率与规模刻度见 `08` §4。
