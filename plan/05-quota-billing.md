...
2. **按调用量算 TTS 是成本大头**：一篇 30 段文章朗读约 $0.07，是同一篇文章 flash 档文本处理的 6 倍。TTS 必须单独限额、强缓存、并在免费档位收紧。
   但**按单次调用算，中档模型的语法与测验更贵**：一段语法分析 $0.007 是一段朗读的 3 倍，一次测验 $0.024 是一段朗读的 10 倍。表里"中档"那一列不是可选项，是成本模型的另一半。
3. 阶段 M2 上线时前两周的实测数据（每活跃用户每日 credits、TTS 占比、缓存命中率）决定 M3 的定价，本文的数字只用于设定初始敞口。
4. **免费档按现有档位买不起这两个功能**：每日 200 credits，语法一次 70、测验一次 240，免费用户一天最多做 2 次语法、永远做不了当天的测验（要攒 3 天到 600 上限才够一次）。而语法与测验恰恰是词典类竞品做不到的差异点。⚠ 三种解法：免费档的语法 / 测验走 flash 档（7 / 24 credits）而 Pro 走中档；或免费档不含这两个功能；或提高每日发放。`08` §3 建议第一种，请确认（决策门 6）。
...
> 档位只回答"给多少"，没有回答"Pro 比 Free 好在哪"。价值阶梯、单位经济与盈亏平衡转化率见 `08` §3–§4；定价本地化（东南亚 / 南亚市场的折扣价）见 `08` §4.5，⚠ 决策门 7。
...
- 发票由 Stripe 自动发送。
- **拒付（chargeback）**：每笔争议手续费约 $15，且拒付率是支付通道的风控指标。`charge.dispute.created` webhook → 立即 `ACCOUNT_SUSPENDED`（保留数据，禁止托管请求），争议解决后人工恢复。服务条款写明。
- **Price 的 `tax_behavior`**：欧盟习惯含税价、美国习惯税前价。用 Stripe 的按国家多 Price 时可分别设 `inclusive` / `exclusive`；单一 USD Price 时设 `exclusive`，欧盟用户在 Checkout 里看到加 VAT 后的总价。
- **税务登记不是 Stripe Tax 的事，是你的事。** 非欧盟主体向欧盟消费者卖数字服务，从第一笔起就要通过 Non-Union OSS 登记并季度申报；英国同样从第一笔起；日本消费税有约 ¥1,000 万门槛。Stripe Tax 只负责算，登记、申报、缴纳都要自己做或找会计。这是 `08` §7.1 建议初期用 Merchant of Record（Paddle / Lemon Squeezy / Polar，卖家是它，税务它管）的核心原因，⚠ 决策门 2。若改 MoR，本节的对象与 webhook 名称按其文档替换，账本设计不变（`ledger.ref` = 支付方事件 ID）。
