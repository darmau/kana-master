...
> 基于本文制定的可执行技术方案与开发计划见 [`plan/README.md`](plan/README.md)。商业化项目正式命名为 **Rubify**（账号/计费/后端品牌，扩展本身在商店里仍叫「読める」Yomeru），下文的"网关""服务器"等表述在 `plan/` 里统一改用这个名字，本文不再逐处回改。
...
4. **收款通道**：Stripe 直连意味着自己承担欧盟 OSS 等税务登记与拒付；Merchant of Record 把这些接走，费率差对海外用户不到 $0.3 / 单。见 `plan/08-business-case.md` §7.1。
5. **仓库公开与许可**：仓库公开、无 LICENSE 文件，后端进同一仓库即开源。见 `plan/08` §7.4。
6. **双品牌**（Yomeru 扩展 / Rubify 账号）的转化代价。见 `plan/08` §7.5。
7. **免费档是否包含语法 / 测验**、定价本地化、Google 以外的登录方式、各里程碑的量化放行标准。见 `plan/README.md` 决策门 6–11。

> 商业面的完整评审（目标用户、竞品、单位经济、增长渠道、法务税务）见 `plan/08-business-case.md`；本文 §4 的成本讨论只覆盖了上游成本，没有算到手收入、免费用户补贴与盈亏平衡转化率。
